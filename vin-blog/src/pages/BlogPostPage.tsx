...
import type { Comment, Blog } from '../types';
import SocialShare from '../components/SocialShare';

interface Props { blogId: string; }

const fd = (d: string, long = false) =>
  new Date(d).toLocaleDateString('en-US', long
    ? { year: 'numeric', month: 'long', day: 'numeric' }
    : { month: 'short', day: 'numeric' });

const sampleContent = (cat: string) => `
<p>When we think about <strong>${cat}</strong> at scale, the architecture decisions we make early define success or failure.</p>
...
<p><strong>Separation of concerns</strong> keeps components focused. <strong>Loose coupling</strong> lets you scale pieces independently.</p>
<h2>Practical Implementation</h2>
<p>Start with clear boundaries and let your architecture evolve as understanding deepens.</p>
<h2>Conclusion</h2>
<p>Great ${cat.toLowerCase()} work comes down to discipline, empathy for your future self, and willingness to refactor when context changes.</p>`;
...
export default function BlogPostPage({ blogId }: Props): JSX.Element {
  ...
  const { data: relatedData } = useGetBlogsQuery({ limit: 50 });
  ...
  useEffect(() => {
    if (blogId) void trackView(blogId);
  }, [blogId]);
  ...
  const handleComment = async (e: FormEvent): Promise<void> => {
    ...
    try {
      ...
      setLocalCmts(p => [...p, {
        ...
        author:    cmtName,
        avatar:    `https://i.pravatar.cc/150?u=${cmtName}`,
        content:   cmtText,
  ...
  if (isError || !blog) {
    return (
      <div className="text-center py-24 px-4">
        ...
        <p className="text-muted mb-6 text-sm">Make sure the backend is running.</p>
  ...
  // Related articles — same category, exclude current blog
  const relatedArticles: Blog[] = (relatedData?.data ?? [])
    .filter((b: Blog) => b.category === blog.category && b.id !== blog.id)
    .slice(0, 2);

  const ACTION_BUTTONS = [
    { icon: liked ? '❤️' : '🤍', label: String(likeCount),      action: handleLike, active: liked },
    { icon: '💬',                 label: String(comments.length), action: undefined,  active: false },
    { icon: '🔖',                 label: 'Save',                  action: undefined,  active: false },
    ...
    <main className="max-w-[1400px] mx-auto px-4 sm:px-6 lg:px-12 py-8">
      <div className="flex flex-col xl:flex-row gap-8 xl:gap-10 items-start">
        ...
        <div className="xl:w-14 xl:flex-shrink-0 xl:sticky xl:top-24 self-start">
          ...
          <div className="flex xl:hidden gap-2 mb-6">
            {ACTION_BUTTONS.map((item, i) => (
              <button
                key={i}
                onClick={() => item.action && void item.action()}
                className={`flex items-center gap-1.5 px-3 py-2 rounded-xl border transition-all cursor-pointer font-body text-xs ${
                  item.active ? 'border-accent bg-accent/10' : 'border-border bg-surface hover:border-accent'
                }`}
              >
                <span>{item.icon}</span>
          ...
          <div className="hidden xl:flex flex-col gap-2">
            {ACTION_BUTTONS.map((item, i) => (
              <button
                key={i}
                onClick={() => item.action && void item.action()}
                className={`flex flex-col items-center gap-1 py-2.5 rounded-xl border transition-all cursor-pointer font-body w-14 ${
                  item.active ? 'border-accent bg-accent/10' : 'border-border bg-surface hover:border-accent'
                }`}
              >
                <span className="text-base">{item.icon}</span>
                <span className="text-[0.57rem] text-muted font-semibold">{item.label}</span>
              </button>
            ))}
          </div>
        </div>
        ...
        <article className="flex-1 min-w-0">
          ...
          {/* Category badge */}
          <span className="inline-block text-[0.6rem] font-bold tracking-widest uppercase text-accent2 px-3 py-1 rounded-full border border-accent2/30 bg-accent2/8 mb-3">
            {blog.category}
          </span>

          {/* Title */}
          <h1 className="font-display font-bold text-2xl sm:text-3xl lg:text-4xl leading-tight text-white tracking-tight mb-4">
            {blog.title}
          </h1>

          {/* Description */}
          <p className="text-muted text-sm sm:text-base leading-relaxed mb-6 italic pl-4 border-l-4 border-accent">
            {blog.description}
          </p>
          ...
          <div className="flex flex-wrap items-center justify-between gap-4 py-4 border-y border-border mb-7">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <div className="text-xs text-muted">{blog.author.bio}</div>
              </div>
            </div>
          ...
          <div className="mt-12 p-6 sm:p-8 rounded-2xl bg-gradient-to-br from-accent/10 to-accent2/8 border border-accent/20 text-center">
            ...
            {subDone ? (
              <p className="text-accent font-semibold">✅ You&apos;re subscribed!</p>
            ) : (
              <div className="flex flex-col sm:flex-row gap-2 max-w-sm mx-auto">
                <input
                  ...
                  onKeyDown={e => e.key === 'Enter' && void handleSub()}
                />
                <button onClick={() => void handleSub()} className="btn-primary whitespace-nowrap">
                  Subscribe
                </button>
              </div>
            )}
            ...
            <div className="divide-y divide-border">
              {comments.map(c => (
                <div key={c.id} className="flex gap-3 py-4">
                  <img
                    src={c.avatar ?? `https://i.pravatar.cc/150?u=${c.author}`}
                    alt={c.author}
          ...
          <div className="mt-14">
            ...
            {relatedArticles.length > 0 ? (
                ...
                {relatedArticles.map((item: Blog) => (
                  ...
                  >
                    ...
                    <div className="p-5 flex-1 flex flex-col">
                      <span className="text-[0.65rem] font-bold uppercase tracking-wider text-accent2">
                        {item.category}
                      </span>
                      <h3 className="font-display font-bold text-white text-lg mt-1.5 mb-2 line-clamp-2 leading-tight">
                        {item.title}
                      </h3>
                      <div className="mt-auto flex items-center gap-2 text-xs text-muted pt-4 border-t border-border">
                        <img src={item.author.image} alt={item.author.name} className="w-5 h-5 rounded-full flex-shrink-0" />
                        <span>{item.author.name}</span>
                        <span className="text-border">•</span>
                        <span>{fd(item.publishedAt)}</span>
                        <span className="text-border">•</span>
                        <span>{item.minsRead}m read</span>
                      </div>
                    </div>
                  </div>
                ))}
              ...
              <p className="text-center text-muted text-sm mb-6">No related articles found in this category.</p>
            )}
            ...
            <p className="text-center text-muted text-sm">
              ...
              <button
                ...
                className="text-accent underline bg-transparent border-0 cursor-pointer font-body text-sm hover:text-white transition-colors"
              >
                the full collection →
              </button>
            </p>
        ...
        <aside className="hidden xl:flex flex-col gap-5 w-56 flex-shrink-0 sticky top-20 self-start">
          <div className="bg-surface border border-border rounded-xl p-4">
            ...
            {['Why This Matters', 'Core Principles', 'Practical Implementation', 'Conclusion'].map(s => (
              <div key={s} className="text-xs text-muted py-2 border-b border-border last:border-0 cursor-pointer transition-colors hover:text-accent">
                {s}
              </div>
            ))}
          </div>
          ...
          <div className="bg-gradient-to-br from-accent/10 to-accent2/8 border border-accent/20 rounded-xl p-4 text-center">
            ...
            <p className="text-[0.63rem] text-muted mb-3 leading-relaxed">Weekly articles in your inbox.</p>
            {subDone ? (
              <p className="text-accent text-xs font-semibold">✅ Subscribed!</p>
            ) : (
              <>
                <input
                  ...
                  onChange={e => setNlEmail(e.target.value)}
                />
                <button onClick={() => void handleSub()} className="btn-primary w-full text-xs py-2">
                  Subscribe
                </button>
              </>
            )}
