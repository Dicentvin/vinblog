...
export default function BlogPostPage({ blogId }: Props): JSX.Element {
  ...
  if (isError || !blog) return (
    <div className="text-center py-24 px-4">
      ...
      <p className="text-muted mb-6 text-sm">The article may not exist or there was a connection error.</p>
  ...
  // Author image — fallback to local file if DB has old URL
  const authorImg = blog.author.image?.includes('vinblog-3m6f') || !blog.author.image
    ? '/authorimg.png'
    : blog.author.image;
      ...
      <div className="flex flex-col xl:flex-row gap-8 xl:gap-10 items-start">
        ...
        <article className="flex-1 min-w-0">
          ...
          <div className="flex flex-wrap items-center justify-between gap-4 py-4 border-y border-border mb-7">
            <div className="flex items-center gap-3">
              <img
                ...
                onError={e => { (e.currentTarget as HTMLImageElement).src = '/authorimg.png'; }}
              />
              <div>
          ...
          <div className="mt-12 p-6 sm:p-8 rounded-2xl bg-gradient-to-br from-accent/10 to-accent2/8 border border-accent/20 text-center">
            ...
            <p className="text-muted text-sm mb-5">Get more like this every week. No spam, unsubscribe anytime.</p>
            {subDone ? (
                ...
                {relatedArticles.map((item: Blog) => (
                  ...
                  >
                    ...
                    <div className="p-5 flex-1 flex flex-col">
                      <span className="text-[0.65rem] font-bold uppercase tracking-wider text-accent2">{item.category}</span>
                      <h3 className="font-display font-bold text-white text-base mt-1.5 mb-2 line-clamp-2 leading-tight">{item.title}</h3>
                      <div className="mt-auto flex items-center gap-2 text-xs text-muted pt-3 border-t border-border">
                        <img
                          src={item.author.image?.includes('vinblog-3m6f') ? '/authorimg.png' : item.author.image}
        ...
        <aside className="hidden xl:flex flex-col gap-5 w-56 flex-shrink-0 sticky top-20 self-start">
          <div className="bg-surface border border-border rounded-xl p-4">
            ...
            {['Introduction', 'Why This Matters', 'Core Principles', 'Conclusion'].map(s => (
              <div key={s} className="text-xs text-muted py-2 border-b border-border last:border-0 cursor-pointer hover:text-accent transition-colors">
                {s}
              </div>
            ))}
          </div>
